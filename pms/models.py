...
from nephr_automations.methods.methods import get_model_class
from nephr_views.cbv_methods import render_template

"""Objectives and key result section"""


class Period(NephrModel):
...
class KeyResult(NephrModel):
    ...
    history = NephrAuditLog(bases=[NephrAuditInfo])
...
class EmployeeObjective(NephrModel):
    ...
    history = NephrAuditLog(bases=[NephrAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
    objects = NephrCompanyManager("employee_id__employee_work_info__company_id")
...
class Comment(models.Model):
    ...
    history = NephrAuditLog(excluded_fields=["comment"], bases=[NephrAuditInfo])
    objects = NephrCompanyManager(
...
class QuestionTemplate(NephrModel):
...
class Question(NephrModel):
    ...
    objects = NephrCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(NephrModel):
    ...
    objects = NephrCompanyManager("question_id__template_id__company_id")


class Feedback(NephrModel):
...
class EmployeeBonusPoint(NephrModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
