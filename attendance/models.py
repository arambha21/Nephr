...
class AttendanceActivity(NephrModel):
    """
...
class BatchAttendance(NephrModel):
    """
...
class AttendanceRequestFile(NephrModel):
    file = models.FileField(upload_to="attendance/request_files")


class AttendanceRequestComment(NephrModel):
    """
...
class AttendanceOverTime(NephrModel):
    """
...
class AttendanceLateComeEarlyOut(NephrModel):
    """
...
class AttendanceValidationCondition(NephrModel):
    """
...
class AttendanceGeneralSetting(NephrModel):
    """
...
if apps.is_installed("leave") and apps.is_installed("payroll"):
    ...
    class PenaltyAccount(NephrModel):
        """
    ...
    def create_initial_stage(sender, instance, created, **kwargs):
        ...
        if created:
            ...
            if penalty_amount:
                Deduction = get_nephr_model_class(
...
class WorkRecords(models.Model):
    ...
    def title_message(self):
        ...
        if title_message == "Absent":
            if apps.is_installed("leave"):
                LeaveRequest = get_nephr_model_class(
